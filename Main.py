...
def start_or_load():
    ...
    if data_exists:
        saved_player = save_and_load.load_data()
    ...
    # if there is no save data then start the intro
        ...
        new_player = Player(user_inputs[0], user_inputs[1])
        new_player.get_stats(new_player.id)
        save_and_load.save_data(new_player)
        #return new_player   
...
while True:
    os.system('cls')
    player = save_and_load.load_data()
    if player.energy <=0:
        break
    ...
    if d == '1':
        ...
        Event_system.library_option(player)
    elif d == "4":
        ...
        i = input(f"{player.hero_name}, {player.power}, {player.mana}. Press enter to close ... ")  
    else:
